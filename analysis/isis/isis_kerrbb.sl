...
variable fbfit = dout + "isis_kerrbb.fit";
...
% Save the TBabs*kerrbb fit parameter file
save_par(fpars);

% Save the TBabs*kerrbb best fit parameters and their 1-sigma uncertainties
fp = fopen(fbfit, "w");
() = fprintf(fp, "Best-fit parameters and 1-sigma range for model: TBabs*kerrbb\n");
() = fprintf(fp, "Mdd       (Mdd_lower, Mdd_upper)  hd      (hd_lower, hd_upper) chisqr     dof\n");
() = fprintf(fp, "%8.6f (%8.6f, %8.6f) %8.6f (%8.6f, %8.6f) %12.6f %3.0f\n",
             Mdd, Mdd_lo[0], Mdd_hi[0], hd, hd_lo[0], hd_hi[0], chisqr, dof);
() = fclose(fp);

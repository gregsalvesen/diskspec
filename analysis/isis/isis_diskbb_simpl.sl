% NAME:
% -----
% isis_diskbb_simpl.sl
%
% PURPOSE:
% --------
% Fit the GRO J1655-40 spectrum with the model "TBabs*simpl*(diskbb)" w/ nH as a fixed parameter.
%
% OUTPUTS: <-- To the directory /Users/salvesen/research/fcolabs/results/
% --------
% isis_diskbb_simpl.par - Parameter file for the model fit
% isis_diskbb_simpl.fit - Bestfit parameters and uncertainties
%
% NOTES:
% ------
...
variable fpars = dout + "isis_diskbb_simpl.par";
variable fbfit = dout + "isis_diskbb_simpl.fit";
...
%====================================================================================================
% FIT MODEL "simpl(TBabs*diskbb)" W/ nH *FIXED* AND OUTPUT RESULTS

% Define the fit function as TBabs*simpl*(diskbb)
fit_fun("TBabs(1)*simpl(1, diskbb(1))");
...
% Save the TBabs*simpl*(diskbb) fit parameter file
save_par(fpars);

% Save the TBabs*simpl*(diskbb) best fit parameters and their 1-sigma uncertainties
fp = fopen(fbfit, "w");
() = fprintf(fp, "Best-fit parameters for model: TBabs*simpl*(diskbb)\n");
() = fprintf(fp, "Kbb (Kbb_lower, Kbb_upper) Tin (Tin_lower, Tin_upper) fScat (fScat_lower, fScat_upper) chisqr dof\n");

...
'''
Modified by Greg Salvesen 2018.
'''
...
class Par:
    ...
    def __init__(self, **argsv):
        ...
        #print(argsv)
        # Greg commented this out --> see parameter_priors() below
    ...
    # Greg moved this --> see parameter_priors() below
...
class Xspec:
    ...
    #----------------------------------------------------------------------------------------------------
    # This is called from xspec_emcee.py
    def parameter_priors(self, parameterpriors):
        """Modify priors to be Gaussian for the parameters specifed in the textfile parameterpriors."""

        # Load in the textfile
        priors   = N.loadtxt(parameterpriors)
        paridx   = priors[:,0].tolist()  # ID numbers for the parameters with Gaussian priors
        parmean  = priors[:,1].tolist()  # Gaussian means
        parsigma = priors[:,2].tolist()  # Gaussian sigmas
        
        # Construct a Gaussian prior
        def gaussPrior(par, pmean, psigma):
            def prior(v):
                if v < par.minval or v > par.maxval:
                    return -N.inf
                # Calculate Gaussian prior log likelihood for parameter
                # (Should we normalize the Gaussian over the min/max values?)
                return N.log(1.0 / (N.sqrt(2.0 * N.pi * psigma**2))) - 0.5 * (v - pmean)**2 / psigma**2
            return prior
        ...
        # Loop through the thawed parameters
        for par in self.thawedpars:

            # Check if this parameter is meant to have a Gaussian prior
            if par.index in paridx:
                print(' Using Gaussian prior for value on parameter %s:%s:%i' % (
                        par.model, par.cmpt, par.index))
                # Find the correct parameter in the list of parameters that have Gaussian priors
                ipar = paridx.index(par.index)
                # Set the Gaussian prior
                par.prior = gaussPrior(par, pmean=parmean[ipar], psigma=parsigma[ipar])

            # Otherwise, assign a flat prior
            #else:
            #    print(' Using flat prior for value on parameter %s:%s:%i' % (
            #            par.model, par.cmpt, par.index))
            #    # Set the flat prior
            #    par.prior = flatPrior(par)
    ...
    def _get_cmpt_pars(self, model, dgrp, cmpt, cmptmodelidx):
        ...
        for paridx in range(startpar, startpar+npars):
            ...
            #----------------------------------------------------------------------------------------------------
            # Covariance matrix <-- work on this later
            #print(p0.tclout('covariance'))
            #----------------------------------------------------------------------------------------------------
